...
export function SkillsListPage() {
  ...
  const skillItems = useLibraryItems({ type: 'skill', query, favoriteOnly });

  const filtered = useMemo(
    () => skillItems.filter((item) => (statusFilter === 'all' ? true : item.status === statusFilter)),
    [skillItems, statusFilter]
  );

  const createSkill = () => {
    const item = createSkillLibraryItem();
    upsertItem(item);
    navigate(`/library/skills/${item.id}`);
  };
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Create, organize, and export structured skills.</p>
        </div>
      ...
      <Card>
        ...
        <CardContent>
              ...
              <tbody>
                {filtered.map((item) => (
                  <tr key={item.id} className="border-b">
                    <td className="p-2">
                      <button className="font-medium hover:underline" onClick={() => navigate(`/library/skills/${item.id}`)}>
                        {item.title}
                      </button>
                    </td>
                    <td className="p-2">{item.status}</td>
                    <td className="p-2">{item.targets.join(', ') || '-'}</td>
                    <td className="p-2">{item.tags.slice(0, 3).join(', ') || '-'}</td>
          ...
          {filtered.length === 0 && <p className="pt-4 text-sm text-muted-foreground">No skills match the current filters.</p>}
        </CardContent>
      </Card>
    </div>
  );
